...
string NaoBehavior::composeAction() {
    ...
    unsigned long role_ass_info = role_ass_num_final;
    bool wantToPassBall = worldModel->getWantToPassBall();
    
    string message;
    if (canTrust || fallen) {
        if(makeSayMessage(uNum, time, timeBallLastSeen, ballX, ballY, myX, myY, fallen, role_ass_info, wantToPassBall, message)) {
            ss << message;
            /*
            cout << "Data Sent:\n";
            cout << "\tTime: " << time << "\n";
