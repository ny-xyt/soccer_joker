...
void IncTar::execute(BodyModel *bodyModel, const WorldModel *worldModel) {
    ...
    for( size_t i = 0; i < effectorIDs.size(); ++i ) {
        bodyModel->increaseTargetAngle( effectorIDs[i], increments[i] );	//increments内保存的是关节的增量
    }
}
...
void KeyFrame::updateCurveMacros(BodyModel *bodyModel, const WorldModel *worldModel, const double &t) {
    ...
    for(size_t i = 0; i < macros.size(); ++i) {
      ...
      //为什么转换？机理是？curve只是用来控制foot？那么其他动作实现？
        Curve *curve = dynamic_cast<Curve *>(macros[i].get());//macros[i].get() 获取该智能指针的原始指针
...
void Skill::execute(BodyModel *bodyModel, const WorldModel *worldModel) {
  ...
  ////cout<<"the currentTime is "<<worldModel->getTime()<<endl;
  ////cout<<"the currentKeyFrame is "<<currentKeyFrame<<endl;
...
//猜想：通过查看.skl文件可知，不同的动作是通过一帧帧动作实现的，当前一帧的动作到达目标位置的时候，在等待时间之后进行下一帧的操作。
//TODO 通过输出keyFrames判断该变量是否在skill变化时清零。ans:经验证，试验结果与猜想相同
