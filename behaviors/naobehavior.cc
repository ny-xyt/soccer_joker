...
NaoBehavior(const std::string teamName, int uNum, const map <string, string> &namedParams_, const string &rsg_) :
    ...
    try {
      ...
      readSkillsFromFile( "./skills/tongda_kick_12m.skl" );
      readSkillsFromFile( "./skills/kick_low.skl" );
      readSkillsFromFile( "./skills/kick_ik_0.skl" );
      readSkillsFromFile( "./skills/kick_short.skl" );
      readSkillsFromFile("./skills/block.skl");
      readSkillsFromFile("./skills/block_new.skl");
      readSkillsFromFile("./skills/fallright.skl");
      readSkillsFromFile("./skills/shoot_UT.skl");
      readSkillsFromFile("./skills/kick_8m.skl");
...
//    cout << "****************************************\n";
    ...
    */
    ...
    if ((worldModel->getPlayMode() == PM_GOAL_LEFT || worldModel->getPlayMode() == PM_GOAL_RIGHT ||
         worldModel->getPlayMode() == PM_BEFORE_KICK_OFF) &&
    ...
    if (getinpassmode == true) action = action + "(pass)";
...
VecPosition
NaoBehavior::collisionAvoidance(bool avoidTeammate, bool avoidOpponent, bool avoidBall, double PROXIMITY_THRESH,
...
VecPosition
NaoBehavior::collisionAvoidanceCorrection(VecPosition start, double PROXIMITY_THRESH, double COLLISION_THRESH,
                                          VecPosition target, VecPosition obstacle) {
    ...
    if (obstacleDist <= COLLISION_THRESH) {
        ...
        VecPosition lrear = start + VecPosition(0, 0, 1).crossProduct(start - obstacle) * 1.0;
        VecPosition rrear = start - VecPosition(0, 0, 1).crossProduct(start - obstacle) * 1.0;
        if (target.getDistanceTo(lrear) > target.getDistanceTo(rrear)) {
            target += (start - obstacle).normalize() * 1.0 + rrear;
        } else {
            target += (start - obstacle).normalize() * 1.0 + lrear;
        }
    }
    return target;
}
...
VecPosition //kick调用
NaoBehavior::collisionAvoidanceApproach(double PROXIMITY_THRESH, double COLLISION_THRESH, VecPosition target,
                                        VecPosition obstacle) {
    return collisionAvoidanceApproach(me, PROXIMITY_THRESH, COLLISION_THRESH, target, obstacle);
}

VecPosition NaoBehavior::collisionAvoidanceApproach(VecPosition start, double PROXIMITY_THRESH, double COLLISION_THRESH,
                                                    VecPosition target, VecPosition obstacle) {
...
bool NaoBehavior::check_float() {
    VecPosition mylastpos = worldModel->getMyLastPosition();
    static bool flaot_time = 0;
    if (me.getDistanceTo(mylastpos) > 3) {
        flaot_time = worldModel->getGameTime();
    }
    if (worldModel->getGameTime() - flaot_time < 2) return 1;
    else return 0;
}//识别球员是否在短时间内移动了很远，以便根据这个状态来调整策略或行为。如果球员在短时间内移动很远，可能需要重新分配角色或采取其他行动来适应比赛情况。
...
SkillType
NaoBehavior::getWalk(WalkRequestBlock::ParamSet paramSet, const double &direction, double rotation, double speed,
...
SkillType NaoBehavior::goToTargetRelative(const VecPosition &targetLoc, const double &targetRot, const double speed,
                                          bool fAllowOver180Turn, WalkRequestBlock::ParamSet paramSet) {
...
SkillType NaoBehavior::goToTarget(const VecPosition &target) {
    ...
    for (unsigned int i = WO_OPPONENT1; i < WO_OPPONENT1 + NUM_AGENTS; i++) {
        WorldObject opp;
        opp.pos = worldModel->getOpponent(i);
        opp.pos.setZ(0);
        if (me.getDistanceTo(opp.pos) < 1) {
            turnAngle = angle;
        }
    }

    // params switch by lyl
    double speed = 1;
    double rot = turnAngle;
    bool fAllowOver180Turn = false;
    WalkRequestBlock::ParamSet paramSet = WalkRequestBlock::PARAMS_DEFAULT;
    static long lastParamDuration = 0;
    static bool canSwitchParam = false;
    const int paramSwitchTresh = 30;//the switch frequency
    lastParamDuration++;
    static WalkRequestBlock::ParamSet lastParamSet = WalkRequestBlock::PARAMS_SPRINT;
    ...
    if ((rot < 15.0 && rot > -15.0) && canSwitchParam) {
        paramSet = WalkRequestBlock::PARAMS_SPRINT;
    }

    if (lastParamSet != paramSet && canSwitchParam) {
        lastParamSet = paramSet;
        lastParamDuration = 0;
        canSwitchParam = false;
    }
    if (lastParamDuration > paramSwitchTresh || paramSet == WalkRequestBlock::PARAMS_SPRINT) {
        canSwitchParam = true;
    }
    ...
    if (agentBodyType != 0)
        paramSet = WalkRequestBlock::PARAMS_DEFAULT;
    return goToTargetRelative(relativeTarget, turnAngle, paramSet);
}
...
bool NaoBehavior::kickPlayMode(int pm, bool eitherTeam) {
    ...
    return pm == PM_CORNER_KICK_LEFT || pm == PM_CORNER_KICK_RIGHT || pm == PM_KICK_IN_LEFT || pm == PM_KICK_IN_RIGHT ||
           pm == PM_FREE_KICK_LEFT || pm == PM_FREE_KICK_RIGHT || pm == PM_DIRECT_FREE_KICK_LEFT ||
           pm == PM_DIRECT_FREE_KICK_RIGHT || pm == PM_GOAL_KICK_LEFT || pm == PM_GOAL_KICK_RIGHT ||
           pm == PM_KICK_OFF_LEFT || pm == PM_KICK_OFF_RIGHT;
}
...
bool NaoBehavior::isIndirectKick(int pm) {
    ...
    return !(pm == PM_DIRECT_FREE_KICK_LEFT || pm == PM_DIRECT_FREE_KICK_RIGHT || pm == PM_CORNER_KICK_LEFT ||
             pm == PM_CORNER_KICK_RIGHT || pm == PM_GOAL_KICK_LEFT || pm == PM_GOAL_KICK_RIGHT);
}
