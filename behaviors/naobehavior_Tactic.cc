...
// 计算机器人与球的角度差
double angleDifference = fabs(robotAngle - ballAngle);
// 设置一个阈值，用于判断机器人是否面对球
double facingThreshold = 30.0;  // 你可以根据需要调整阈值
// 判断是否面对着球
    ...
    target_ball = collisionAvoidance(true/*Avoid teamate*/,false/*Avoid opponent*/,false/*Avoid ball*/, .5, 1, target_ball, false /*fKeepDistance*/);
        //defend_time==true;
           ...
           if(posball.getX()>-15&&posball.getX()<-12.5&&posball.getY()>-3&&posball.getY()<0)
           {
            ...
            {
                if(worldModel->getFallenTeammate(worldModel->getUNum())) //获取我方当前球员是不是倒地
                {
                    if(angleDifference < facingThreshold)
                 {
                   return goToSpace(VecPosition(-15,0,0));
                  }
                  else
                  {
                    faceBall();
...
SkillType NaoBehavior::BACK_RIGHT_Tactic() {
 VecPosition posball = worldModel->getBall();
    ...
    if(posball.getX()<-6.5&&posball.getX()>-15&&posball.getY()<0&&posball.getY()>-10)
    {
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            ifpassmode();
...
{
    VecPosition posball = worldModel->getBall();  
    if(posball.getX()<7&&posball.getX()>5&&posball.getY()<10&&posball.getY()>1)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(TONGDA_KICK_12M, VecPosition(15, 0, 0));
    ...
    else if(posball.getX()<5&&posball.getX()>-7&&posball.getY()<10&&posball.getY()>1)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            ifpassmode();
    ...
    else if(posball.getX()<0)
    {
        if (me.getDistanceTo(getSpaceForRole(agentRoles[worldModel->getUNum()])) < 0.5) 
            ...
            return goToTargetRelative(VecPosition(), atan2Deg(worldModel->g2l(ball).getY(), worldModel->g2l(ball).getX()));
             ...
             // not close to the target
            return goToSpace(VecPosition(-4, 6, 0));
...
{
    VecPosition posball = worldModel->getBall(); 
    if(posball.getX()<7&&posball.getX()>5&&posball.getY()<-1&&posball.getY()>-10)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(TONGDA_KICK_12M, VecPosition(15, 0, 0));
    ...
    else if(posball.getX()<5&&posball.getX()>-7&&posball.getY()<-1&&posball.getY()>-10)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            ifpassmode();
...
SkillType NaoBehavior::MID_MID_Tactic()
{
    ...
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(KICK_8M, VecPosition(5, 5, 0));
        }
        else return cy_findball(MID_MID);
    }
    else if(posball.getX()<0)
...
{
    VecPosition posball = worldModel->getBall(); 
    if(posball.getX()<15&&posball.getX()>3&&posball.getY()<1&&posball.getY()>-10)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(TONGDA_KICK_12M, VecPosition(15, 0, 0));
...
{
    VecPosition posball = worldModel->getBall(); 
    if(posball.getX()<15&&posball.getX()>2&&posball.getY()<7&&posball.getY()>-7)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(TONGDA_KICK_12M, VecPosition(15, 0, 0));
...
{
    VecPosition posball = worldModel->getBall(); 
    if(posball.getX()<15&&posball.getX()>3&&posball.getY()<10&&posball.getY()>1)
    { 
        if(getPlayerClosestToBall()==worldModel->getUNum())
        {
            return kickBall(TONGDA_KICK_12M, VecPosition(15, 0, 0));
