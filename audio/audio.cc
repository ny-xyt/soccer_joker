 ...
 * RoleAssign message - A 34 bits message which provide the Roleassign information  
 * message - string to return say message in
 * RETURN - true if message is created, false otherwise
 */
bool makeSayMessage(const int &uNum, const double &currentServerTime, const double &ballLastSeenServerTime, const double &ballX, const double &ballY, const double &myX, const double &myY, const bool &fFallen, unsigned long &role_ass_info, bool &wantToPassBall ,string &message) {
    ...
    vector<int> bits;
    if(!(dataToBits(currentServerTime, ballLastSeenServerTime, ballX, ballY, myX, myY, fFallen, role_ass_info, wantToPassBall,bits))) {
        return false;
    }
 ...
 */
bool processHearMessage(const std::string &message, const double &heardServerTime, int &uNum, double &ballLastSeenServerTime, double &ballX, double &ballY, double &agentX, double &agentY, bool &fFallen, unsigned long &role_ass_info, bool &wantToPassBall, double &time) {
   ...
   vector<int> bits;
    ...
    if(!(bitsToData(bits, time, ballLastSeenServerTime, ballX, ballY, agentX, agentY, fFallen, role_ass_info, wantToPassBall))) {
        return false;
    }
    
    //cout << "PM " << role_ass_info << endl;
...
vector<int> intToBits(const unsigned  long &n, const int &numBits) {
    vector<int> bits;
...
unsigned long bitsToInt(const vector<int> &bits, const int &start, const int &end) {
...
bool dataToBits(const double &time, const double &ballLastSeenTime, const double &ballX, const double &ballY, const double &myX, const double &myY, const bool &fFallen, unsigned long  &role_ass_info, bool &wantToPassBall, vector<int> &bits) {
    ...
    vector<int> Role_Bits = intToBits(role_ass_info, 34);

    int wantToPassBallBit = wantToPassBall? 1:0;
    ...
    bits.insert(bits.end(), Role_Bits.begin(), Role_Bits.end());//34
    bits.push_back(wantToPassBallBit);//1
...
bool bitsToData(const vector<int> &bits, double &time, double &ballLastSeenTime, double &ballX, double &ballY, double &agentX, double &agentY, bool &fFallen, unsigned long &role_ass_info, bool &wantToPassBall) {
    if(bits.size() < (16 + 16 + 10 + 10 + 10 + 10 + 1 + 34)) {
        time = 0;
        ballLastSeenTime = 0,
        ballX = 0;
        ballY = 0;
    ...
    role_ass_info = bitsToInt(bits, ctr, ctr + 33);
    ctr += 34;

    wantToPassBall = bits[ctr] != 0;
    ctr += 1;
